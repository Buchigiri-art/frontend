...
export function StudentTable({
  ...
  const semesters = useMemo(
    () =>
      Array.from(new Set(students.map((s) => s.semester)))
        .filter(Boolean)
        .sort(),
    [students]
  );
  ...
  const baseColumns = 6; // Name, USN, Email, Branch, Year, Semester
  let columnsCount = baseColumns;
  if (showCheckboxes) columnsCount += 1;
  if (enableActions) columnsCount += 1;
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50">
              ...
              {enableActions && <TableHead className="w-32">Actions</TableHead>}
            </TableRow>
          </TableHeader>
                    ...
                    {showCheckboxes && (
                      <TableCell>
                        <Checkbox
                          ...
                          onCheckedChange={(val) =>
                            handleSelectStudent(student.email, !!val)
                          }
                        />
                      </TableCell>
                    )}
